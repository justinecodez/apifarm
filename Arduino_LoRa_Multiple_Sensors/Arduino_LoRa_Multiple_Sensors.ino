...
void loop(void){
    ...
    float temp = dht.readTemperature(); //SENSOR VALUE 1, DESCRIPTION TC
    float hum = dht.readHumidity();  //SENSOR VALUE 2, DESCRIPTION HM
    float gas = analogRead(gasPin);
    float motion = digitalRead(pin);
      ...
      if(a){
        ...
        c = false;
        d = false;
        postValues("\\!TEMP/%s", temp);
      }else if(b){
        a = false;
        b = false;
        c = true;
        d = false;
        postValues("\\!HUM/%s", hum);
      }else if (c){
        a = false;
        b = false;
        c = false;
        d = true;
        postValues("\\!GAS/%s", gas);
      } else if (d){
        a = true;
        b = false;
        c = false;
        d = true;
        postValues("\\!MOTION/%s", motion);
      }
      ///////////////////////////
}
